...
export class CombiniOAuthProvider implements OAuthProviderInterface {
  public readonly name = "combini";
  public readonly clientId: string;
  public readonly authUrl = "https://combini.ai/_api/oauth/authorize";
  public readonly scopes = "openid email profile";
  public readonly redirectUri: string;
  private readonly clientSecret: string;
  private readonly tokenUrl = "https://combini.ai/_api/oauth/token";
  private readonly userInfoUrl = "https://combini.ai/_api/oauth/userinfo";

  constructor(redirectUri: string) {
    this.clientId = process.env.COMBINI_OAUTH_CLIENT_ID || "";
    this.clientSecret = process.env.COMBINI_OAUTH_CLIENT_SECRET || "";
    this.redirectUri = redirectUri;

    if (!this.clientId) {
      const error = new Error(
        "COMBINI_OAUTH_CLIENT_ID environment variable is required"
      );
      console.error("CombiniOAuthProvider initialization failed:", error);
      throw error;
    }

    if (!this.clientSecret) {
      const error = new Error(
        "COMBINI_OAUTH_CLIENT_SECRET environment variable is required"
      );
      console.error("CombiniOAuthProvider initialization failed:", error);
      throw error;
    }
  ...
  async exchangeCodeForTokens(
    ...
    console.log(
      "CombiniOAuthProvider: Exchanging authorization code for tokens",
      {
    ...
    try {
      ...
      console.error("CombiniOAuthProvider: Token exchange fetch error:", {
        error:
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error(
          "CombiniOAuthProvider: Failed to read error response text:",
          textError
        );
      }

      console.error(
        "CombiniOAuthProvider: Token exchange failed with error response:",
        {
    ...
    try {
      ...
      console.error(
        "CombiniOAuthProvider: Failed to parse token exchange response JSON:",
        {
          error:
    ...
    if (!data.access_token) {
      throw new OAuthError(
        "TOKEN_EXCHANGE_FAILED",
        "No access token received from Combini",
        this.name,
        data
      );
    }
  ...
  async fetchUserInfo(tokens: OAuthTokens): Promise<any> {
    ...
    try {
      ...
      console.error("CombiniOAuthProvider: User info fetch error:", {
        error:
          fetchError instanceof Error ? fetchError.message : String(fetchError),
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorText = "Could not read error response body";
        console.error(
          "CombiniOAuthProvider: Failed to read user info error response text:",
          textError
        );
      }

      console.error(
        "CombiniOAuthProvider: User info fetch failed with error response:",
        {
          status: response.status,
          statusText: response.statusText,
          errorBody: errorText,
    ...
    try {
      ...
      console.error(
        "CombiniOAuthProvider: Failed to parse user info response JSON:",
        {
          error:
            jsonError instanceof Error ? jsonError.message : String(jsonError),
          status: response.status,
  ...
  mapUserData(userInfo: any): StandardUserData {
    ...
    if (!userInfo.id) {
      throw new OAuthError(
        "PROVIDER_ERROR",
        "Combini user info missing required id field",
        this.name,
        userInfo
      );
    }
    ...
    if (!userInfo.email) {
      throw new OAuthError(
        "PROVIDER_ERROR",
        "Combini user info missing required email field",
        this.name,
        userInfo
      );
    }
