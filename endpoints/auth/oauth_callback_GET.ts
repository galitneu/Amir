...
export async function handle(request: Request) {
  try {
    ...
    const oauthStateResults = await db
      ...
      .select(["id", "redirect_url", "expires_at", "code_verifier", "provider"])
      .where("state", "=", state)
      .limit(1)
      .execute();
    ...
    try {
      oauthProvider = getOAuthProvider(providerName, oauthState.redirect_url);
    } catch (providerError) {
    ...
    const adminEmails = ["galiel@gmail.com", "benifeld@gmail.com", "galitneu@gmail.com"];
    const userRole = adminEmails.includes(mappedUserData.email) ? "admin" : "user";
    ...
    if (existingUsers.length > 0) {
      ...
      if (existingProviderAccount) {
        ...
        await db
          ...
          .set({
            provider_user_id: mappedUserData.providerUserId,
            provider_email: mappedUserData.email,
            updated_at: now,
          })
      ...
      await db
        .updateTable("users")
        .set({
          display_name: mappedUserData.displayName,
          avatar_url: mappedUserData.avatarUrl || null,
          role: userRole,
          updated_at: now,
        })
        .where("id", "=", user.id)
